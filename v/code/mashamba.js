...
export async function load_title_1() {
    ...
    const results = await server.exec('database', ['mutall_mashamba', false], 'get_sql_data', ['/mashamba/v/code/mashamba.sql', 'file']);
    //
    //Get the first page element
    const page1 = document.getElementById('first_page');
    //
    //Pages of a record
    const pages = JSON.parse(results[0].pages);
    //
    ...
    const other_pages = document.getElementById('other_pages');
    //
